...
// 底部懸浮狀態列
export default function StatusBar({ stageLabel, teamA, teamB, displayName, isConnected, obsOnline, obsCount }) {
  return (
    <div className="fixed left-4 right-4 bottom-4 z-40">
      <div className="mx-auto max-w-7xl rounded-xl bg-white/90 dark:bg-gray-800/90 shadow-lg ring-1 ring-black/5 backdrop-blur px-4 sm:px-6 py-3 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <span
            title={isConnected ? '伺服器 SSE 已連線' : '伺服器 SSE 未連線'}
            className={`inline-block w-2.5 h-2.5 rounded-full ${isConnected ? 'bg-green-500' : 'bg-red-500'}`}
          />
          <span
            title={obsOnline ? `OBS 在線（${obsCount}）` : 'OBS 離線：請確認 OBS 瀏覽器來源是否仍在執行'}
            className={`inline-block w-2.5 h-2.5 rounded-full ${obsOnline ? 'bg-green-500' : 'bg-yellow-500'}`}
          />
